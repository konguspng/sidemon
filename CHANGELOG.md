...
- Fixed: the auto-fit check ran before the drive/monitor list had actually finished rendering, so it measured a near-empty layout and never detected real overflow (e.g. enabling more drives in the Monitors tab). It now waits for layout to genuinely settle before measuring.
