...
- "Toolbar Mode" is renamed "Desktop Widget Mode" (same setting, still on by default) with a clearer description: SideMon behaves like a desktop gadget - off the taskbar, out of Alt+Tab, shown on every virtual desktop - unless you turn it off to treat it as a normal switchable window.
- Turning off "Always On Top" now makes the sidebar a true desktop-level widget: it sits pinned at the bottom of the window stack so ordinary windows always cover it, and it lifts back into view only while the desktop itself is brought forward (Win+D / "Show desktop"). Previously an un-pinned sidebar floated loose in the middle of the z-order. "Always On Top" on (the default) is unchanged.
- Fixed: the FPS overlay could be Alt+Tab'd to and appeared in the task switcher. It's now a proper tool window (set before it's first shown), so it stays out of the switcher like the sidebar. Its click-through style is applied at the same point.
- Fixed: enabling/disabling individual metrics (Monitors tab) took effect on the live settings immediately and could not be undone with Cancel - the Settings dialog was editing the same metric objects as the running app instead of a private copy. It now edits a real copy, so Cancel discards metric changes and only Save/Apply commits them. This also closes a path where an unrelated automatic settings save (such as the auto-fit UI-scale writer) could quietly persist metric changes you had cancelled.
...
- Fixed: the installer could freeze on "Setup is preparing to install SideMon on your computer" while an older version was still running. This came from Inno Setup's automatic Restart Manager check (`CloseApplications=yes`), which can stall waiting on a running app's window to respond; the installer already force-closes the app itself via `taskkill /f` right before copying files, so the redundant automatic check is now disabled.
